...
export function createReferenceCampaignModule({ cycleValidator, referenceRun, ui }) {
    ...
    async function simulateDeterministically(engine, {
        maximumSeconds,
        beforeStep = null,
        afterStep = null,
        stopWhen = null,
    ...
    async function startFreshEngine(engine, {
        progressStart,
        progressEnd,
        phaseLabel
    }) {
    ...
    async function runDeterministicReferenceSweep() {
        ...
        const elapsed = await simulateDeterministically(engine, {
            ...
            progressStart: 9,
            progressEnd: 35,
            onProgress: (progress, simulatedSeconds) => setReferenceRunUi({
                phase: "Tir déterministe",
                progressPercent: progress,
    ...
    function coefficientOfVariationPercent(values) {
        ...
        const average = finiteValues.reduce((sum, value) => sum + value, 0)
            / finiteValues.length;
    ...
    function cycleStabilityCvPercent(cycles) {
    ...
    function cycleBoostSpanBar(cycles) {
        const values = (cycles ?? [])
            .map(cycle => cycle?.summary?.meanBoostBarGauge)
            .filter(Number.isFinite);
        if (values.length < 2) return Infinity;
        return Math.max(...values) - Math.min(...values);
    }

    function telemetryWindowByDuration(samples, durationSeconds) {
        ...
        return finiteSamples.filter(sample => sample.time >= minimumTime);
    }

    function telemetryTimeSpanSeconds(samples) {
    ...
    function telemetryAbsoluteSlopePerSecond(samples, key) {
    ...
    function telemetryRelativeSpanPercent(samples, key) {
    ...
    function getSlowSystemStability(samples) {
    ...
    function restoreStateSnapshot(engine, snapshot, baseStepDeg) {
    ...
    function maximumAbsoluteTelemetryValue(samples, key) {
        let maximum = 0;
        let found = false;
    ...
    function buildOperatingPointResult({
        ...
        const meanRpm = cycles.reduce(
            (sum, cycle) => sum + finite(cycle?.summary?.meanRpm),
            0
        ) / cycles.length;
        const meanBoostBarGauge = cycles.reduce(
            (sum, cycle) => sum + finite(cycle?.summary?.meanBoostBarGauge),
            0
        ) / cycles.length;
    ...
    async function runSingleOperatingPoint({
        ...
        let latestRepeatabilityStatus = CYCLE_VALIDATION_STATUS.UNAVAILABLE;
        ...
        await simulateDeterministically(engine, {
            ...
            beforeStep: ({ state, elapsedSeconds, stepSeconds }) => {
    ...
    async function runMultiPointValidationCampaign(referenceValidation, {
        progressStart = 60,
        progressEnd = 88
    } = {}) {
        const points = MULTI_POINT_PROTOCOL.points;
